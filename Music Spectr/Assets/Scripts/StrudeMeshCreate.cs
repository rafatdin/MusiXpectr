...
public class StrudeMeshCreate : MonoBehaviour 
{
    ...
    protected MeshCollider collider = null;
	protected void init()
	{
	...
	public static Vector3[] conPoints;

    void generatePoints()
    {
        float[] samples = GlobalData.samples; //receive data from previous scene;
        float[] averages = GlobalData.processedSamples;
        //iniate control points
        //samples are divided by 88400 gives us the length of audiofile in seconds
        conPoints = new Vector3[samples.Length / 88400 + 1];

        for (int i = 0; i < conPoints.Length; i++)
        {
            conPoints[i] = new Vector3(i * 60, averages[i]*1000,0);
        }
    }

    //here is the catmull-rom evalution.
    float catEval(float p0, float p1, float p2, float p3, float t)
	{
		float t2 = t*t;
	...
	//vector version.
	Vector3 catEval(Vector3 p0,Vector3 p1,Vector3 p2,Vector3 p3,float t)
	{
		float t2 = t*t;
		return 0.5f * ( (2.0f * p1) + (-p0 + p2)*t + 
				      ( 2.0f*p0 - 5.0f*p1 + 4.0f*p2 - p3)*t2 
